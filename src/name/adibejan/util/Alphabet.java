...
public class Alphabet<T> implements java.io.Serializable {
  ...
  /**
   * Returns the corresponding item for a specified <code>index</code>.
   * The index must be in the boundary limits.
   */
  public T getItem(int index) { 
    if(index < 0 || index >= size) throw new ArrayIndexOutOfBoundsException("The index ["+index+"] is outside the alphabet bounds!");    
  ...
  /**
   * Reads an alphabet from a specific location
   * if the location is not valid or there is no alphabet at that location, create a new alphabet
   */
