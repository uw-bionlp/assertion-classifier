...
public class DoublePair implements Serializable {
  ...
  /**
   * Constructs an <tt>DoublePair</tt> object from anoter <tt>DoublePair</tt> object
   */
  ...
  public boolean equals(Object that) {
    ...
    if ( !(that instanceof DoublePair) ) return false;
   ...
   * It does not always ensures uniqueness due to the loss of precission in conversion
   */
  @Override
  public int hashCode() {
  ...
  /**
   * Test whether this <tt>DoublePair</tt> object is a point (i.e., its values are equal).
   *
   */
  ...
  /**
   * Test whether this <tt>DoublePair</tt> object has its values in the increasing order
   *
   */
  ...
  /**
   * Test whether this <tt>DoublePair</tt> object has its values in the strictly increasing order
   *
   */
  ...
  public double compute(ArOp2 op) {
    ...
    case DIV: 
      if(second == 0) throw new ArithmeticException("The second value in the pair is zero!");
      return first / second; 
  ...
  public static Comparator<DoublePair> getComparatorByFirstValue() {
    return new Comparator<DoublePair>() {
      public int compare(DoublePair dp1, DoublePair dp2) {
        if(dp1.getFirst() < dp2.getFirst()) return -1;
        if(dp1.getFirst() > dp2.getFirst()) return 1;
        return 0;
      }
    };
  ...
  /**
   * Compares the values of two <code>DoublePair</code> objects by their first values
   */
  public int compareByFirstValue(DoublePair other) {
    if(getFirst() < other.getFirst()) return -1;
    if(getFirst() > other.getFirst()) return 1;
    return 0;
  }
  ...
  public static Comparator<DoublePair> getComparatorBySecondValue() {
    return new Comparator<DoublePair>() {
      public int compare(DoublePair dp1, DoublePair dp2) {
        if(dp1.getSecond() < dp2.getSecond()) return -1;
        if(dp1.getSecond() > dp2.getSecond()) return 1;
        return 0;
      }
    };
  ...
  /**
   * Compares the values of two <code>DoublePair</code> objects by their second values
   */
