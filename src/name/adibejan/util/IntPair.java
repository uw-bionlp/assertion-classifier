...
/**
 * Tool class for an (int, int) pair. This class can also be seen as a class for
 * interval objects.
 *
...
public class IntPair implements Comparable<IntPair>, java.io.Serializable {
    ...
    /**
     * Creates a int-int pair from a string representaion. In the string
     * representation the int values are separated by a specified delimiter.
    ...
    public static IntPair getInstance(String rep, String delim) {
        ...
        if (pos == -1)
            throw new UnsupportedDataFormatException("[" + delim + "] does not exist in [" + rep + "]");
    ...
    /**
     * Test whether this <tt>IntPair</tt> object is a point (i.e., its values are
     * equal).
     *
     */
    ...
    /**
     * Test whether this <tt>IntPair</tt> object has its values in the increasing
     * order
     *
     */
    ...
    /**
     * Test whether this <tt>IntPair</tt> object has its values in the strictly
     * increasing order
     *
     */
    ...
    public static Comparator<IntPair> getDescComparatorByDistanceAbs() {
        return new Comparator<IntPair>() {
            public int compare(IntPair i1, IntPair i2) {
                if (i2.distanceAbs() < i1.distanceAbs())
                    return -1;
                if (i2.distanceAbs() > i1.distanceAbs())
                    return 1;
                return 0;
    ...
    public TemporalRelation getTemporalRelation(IntPair interval) {
        ...
        if (ithis.second < ithat.first)
            return TemporalRelation.BEFORE;
        if (ithat.second < ithis.first)
        ...
        if ((!ithis.isPoint() && !ithat.isPoint())) {
            if (ithis.first == ithat.first && ithis.second < ithat.second)
                return TemporalRelation.STARTS;
            if (ithat.first == ithis.first && ithat.second < ithis.second)
     ...
     *         1 for AFTER, MET_BY, OVERLAPPED_BY, STARTS, CONTAINS, and
     *         FINISHED_BY;<br/>
    ...
    /**
     * Returns the absolute value of the distance between the int values of this
     * object
     *
     */
    ...
    public boolean isIncluded(IntPair interval) {
        if (interval.first <= first && second <= interval.second)
            return true;
        return false;
    }
    ...
    public boolean includes(IntPair interval) {
        if (first <= interval.first && interval.second <= second)
            return true;
        return false;
    }
    ...
    public boolean includes(int interval) {
        if (first <= interval && interval <= second)
            return true;
        if (second <= interval && interval <= first)
            return true;
        return false;
    }
     ...
     * 
     * @return the position in the list of the first interval that intersects a
     *         specified interval. If the specified interval does not intersects any
     *         of the intervals in the list, it returns -1.
     */
    ...
    /**
     * Insert a specific interval into a list of intervals. The operation is
     * performed such that the resulting intervals will have the largest span and
     * none of them intersect with each other.
     *
     * @assumption all the intervals in the <code>intervals</code> list do not
