...
public class IntSet { 
  /* the resaon for not using the TIntHashSet class is because of missing 
     some basic functionalities such as size(), isEmpty(), and get() which would 
  ...
  public IntSet(int MAX, boolean fromZero) {
    ...
    for(int elem = start; elem <= MAX; elem++) set.add(elem);
  }  
  ...
  /**
   * Creates a <tt>IntSet</tt> instance of a specified size. The elements 
   * the new set are consecutive with the smallest element starting from a 
  ...
  public static IntSet getInstance(int init, int size) {
    IntSet s = new IntSet();
    for(int i = 0; i < size; i++) s.add(init + i);
    return s;    
  }
  ...
  public boolean isStrictlyIncluded(IntSet otherSet) {
    ...
    for(int i = 0; i < set.size(); i++)
      if(!otherSet.contains(set.get(i))) return false;
    return true;
  }
  ...
  public IntSet symmetricDifference(IntSet otherSet) {
    if(otherSet.isEmpty()) return this.clone();
    if(isEmpty()) return otherSet.clone();
    
    IntSet sd = new IntSet();
    for(int i = 0; i < this.size(); i++)
      if(!otherSet.contains(this.get(i))) sd.add(this.get(i));
    for(int i = 0; i < otherSet.size(); i++)
      if(!this.contains(otherSet.get(i))) sd.add(otherSet.get(i));
